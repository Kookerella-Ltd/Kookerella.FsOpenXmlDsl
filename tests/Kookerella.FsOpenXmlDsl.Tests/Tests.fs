module Kookerella.FsOpenXmlDsl.Tests
...
/// `dotnet fsi` needs `#r` for both Kookerella.FsOpenXmlDsl itself and its OpenXml SDK dependency -
...
/// Kookerella.FsOpenXmlDsl's own `Writer` never emits Excel's "shared formula" optimization (it always
...
let ``reading a foreign file's shared formulas reconstructs each cell's own formula`` () =
    let path = Path.Combine(Path.GetTempPath(), sprintf "Kookerella.FsOpenXmlDslSharedFormulaTest_%s.xlsx" (Guid.NewGuid().ToString("N")))
