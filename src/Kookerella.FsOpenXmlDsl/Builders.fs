...
/// this case now that `Writer` also has `open Kookerella.FsOpenXmlDsl` in scope. `Reader`'s
...
/// (that's a member-only feature), so this needs a type. `open type Kookerella.FsOpenXmlDsl.SheetDsl`
/// alongside `open Kookerella.FsOpenXmlDsl` brings `cell`/`row` into scope unqualified, the same
